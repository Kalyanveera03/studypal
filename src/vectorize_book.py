...
vector_db_dir = f"{parent_dir}/vectordb"
...
def vectorize_book_and_store_to_db(class_subject_name,vector_db_name):
    ...
    loader = DirectoryLoader(path=book_dir,glob="./*.pdf",loader_cls=UnstructuredPDFLoader)
...
def vectorize_chapters(class_subject_name):
    ...
    for chapter in os.listdir(book_dir):
     ...
     loader = UnstructuredPDFLoader(chapter_pdf_path)
     documents = loader.load()
